...
impl LocalStore {
    pub fn open() -> Result<Self, String> {
        ...
        let root = PathBuf::from(home).join(".ai-gateway-client");
        Self::open_at(root, true)
    }

    fn open_at(root: PathBuf, apply_environment: bool) -> Result<Self, String> {
        ...
        let config = if config_path.exists() {
            ...
            // intentionally ignored. Provider state now belongs exclusively
            // to local-core's encrypted SQLite database.
...
mod tests {
    ...
    fn unique_test_dir() -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir().join(format!("ai_gateway_tauri_store_{unique}"))
    }
}
