...
- 手动创建账号：管理员可在“管理员设置 → 用户管理”中创建邮箱 + 密码账号（指定角色 admin / user），用于调试，或为没有飞书的成员开通登录。密码使用 Argon2id（64 MiB 内存、3 次迭代）加随机盐存储，不保存明文。手动账号在 `required` 模式下可通过登录页的“邮箱密码登录”进入，并同样拥有自己的 API Key。
